...
public class DecodeHandler {
    ...
    private volatile MusicMessage musicMessage;
...
//            System.out.println(streamReady + ", " + touchedEndOfStream);
            ...
            if (musicMessage == null && isPaused) return;
            ...
            if (!streamReady) {
                synchronized (SEEK_LOCK) {
                    ...
                    if (buildInputStream()) {
                        AudioDecoder decoder = decoders.get(audioMessage.encoding);
                        this.audioMessage = decoder.readyAudioStream(dataStream, dataStreamSize, is -> this.dataStream = is);
                        this.dataStream = decoder.decode(dataStream, audioMessage);
    ...
    private boolean buildInputStream() throws AudioDecodeException {
        UniversalPath.UrlPath path = musicMessage.audioPath.getPath();
        ...
        try {
            switch (path.type()) {
                case NATIVE -> {
                    ...
                    if (audioMessage == null) this.audioMessage = new AudioMessage();
                    this.audioMessage.encoding = detectionResult.encoding();
                    this.musicMessage.audioMessage = this.audioMessage;
